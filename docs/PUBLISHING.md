...
`ReleasePackage.py` remains the local-only source hygiene/packaging helper. `ReleaseCandidateHardening.py` is the stricter RC source and Windows-artifact gate.
...
After the gates pass, the workflow creates a draft versioned GitHub Release, uploads the installer, portable ZIP, checksums and manifest, then publishes it. Published versions are not overwritten. Draft upload retries require the same commit. Always use a new version number for the next patch.

The in-app updater reads published Releases, not Actions artifacts or branch commits. It requires the correctly named installer with a GitHub SHA-256 digest. See [in-app updates](IN-APP-UPDATES.md).
