...
- The selected canvas is reused only when it still belongs to the same Paint window and the client size is unchanged. If Paint only moved, the selection is safely rebased. If Paint resized or the target changed, Image Draw Bot falls back to fresh detection.
...
- Automatic detection keeps the full document envelope so a black/covered region inside the canvas cannot make Image Draw Bot choose a smaller white sub-area.
